...
use ast_model::{self as ast, SolType, operators::UnaryOperatorKind};
use mir_model as mir;
use sol_utils::{
    Mutable, TypeModifier, compiler_options::MirOptions, fault::Fault,
    intrinsics::IntrinsicFunction, sol_error_internal, sol_names::PrimitiveTypes, span::Span,
};
...
impl<'a> FunctionLowerer<'a> {
    ...
    fn lower_new_expression(
        ...
        let SolType::Pointer(reference) = &ptr_ty else {
            return Err(sol_error_internal!(
                "new(expr)'s own expression type is always SolType::Pointer",
                Some(span)
            )
            .into_kind());
        };
