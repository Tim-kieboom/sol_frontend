...
use ast_model::{self as ast, SolType, declare_store::DeclareStore};
use mir_model as mir;
use sol_utils::{fault::Fault, span::Span};

impl<'a> FunctionLowerer<'a> {
    /// The Sol type held at `place`, walking its projection the same way
    ...
    /// The Sol type an already-lowered operand carries, if any — a bare
    /// constant carries no type of its own (mirrors `mir_codegen::rvalue`'s
    /// `operand_type`, one layer up: Sol types here, LLVM types there).
    ...
    pub(super) fn operand_type(&self, operand: &mir::Operand) -> Option<SolType> {
        match operand {
    ...
    pub(super) fn is_float_operand(
        ...
        matches!(ty, Some(SolType::Primitive(p)) if p.is_float())
    }
    ...
    pub(super) fn resolve_struct(&self, ty: &SolType) -> Option<&ast::Struct> {
