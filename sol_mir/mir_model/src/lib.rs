...
use ast_model::{
    Literal, SolType, TypeId, declare_store::DeclareStore, operators::BinaryOperatorKind,
};
use sol_utils::{
...
impl_sol_ids!(LocalId, BlockId);

/// The type of a MIR local. An interned handle onto the frontend's resolved
/// type — resolve back to `&SolType` via `DeclareStore::get_type` wherever
...
pub enum Rvalue {
    ...
    /// `SolType::Pointer` (`*T`), an *owning* pointer freed by its own
    /// `Drop` (see M2's TODO.md entry), unlike a plain `Ref`. Doesn't fit
    /// `Terminator::Call`, which requires a real Sol-level `FunctionId` —
...
impl PlaceElem {
    /// The Sol type reached by stepping through this one projection element
    ...
    /// `Deref` (delegates to `SolType::deref_once`). `None` if `ty` doesn't
    ...
    /// Sol-type re-derivation of an already-built `Place`) and
    ...
    pub fn step_type(
        ...
        match self {
            PlaceElem::Field(index) => {
                if let SolType::TupleKind(ast_model::TupleKind::Tuple(types)) = ty {
            ...
            PlaceElem::Index(_) => {
                let SolType::Array(array) = ty else {
                    return None;
                };
