...
/// (`main`) has to match the Sol function's actual declared name exactly.
...
impl<'ctx, 'a> ModuleCodegen<'ctx, 'a> {
    ...
    /// struct-typed `SolType::Stub`'s bare name back to its declaration.
    ...
    fn declare_all_functions(&mut self) -> CodegenResult<()> {
        for (id, function) in self.mir.functions.entries() {
            ...
            // the process exit code) regardless of Sol's declared return
