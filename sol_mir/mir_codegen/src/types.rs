//! Sol-type-to-LLVM-type mapping and the small constant/int-width helpers
...
use ast_model::{ArrayKind, SolType, TupleKind, declare_store::DeclareStore};
use inkwell::{
    AddressSpace,
    context::Context,
...
/// Maps a Sol type to its LLVM representation. Integers/`bool` map to the
...
pub(crate) fn llvm_type<'ctx>(
    ...
    use PrimitiveTypes::*;
    match ty {
        SolType::Primitive(prim) => Ok(match prim {
        ...
        SolType::Reference(_) | SolType::Pointer(_) | SolType::RawPtr(_) => {
            Ok(context.ptr_type(AddressSpace::default()).into())
        }
        SolType::Stub(_) => stub_type(context, platform, declares, module, ty, span),
        SolType::Array(array) => array_type(context, platform, declares, module, array, span),
...
fn array_type<'ctx>(
    ...
    if !array.kind.is_lowerable() {
        return Err(Fault::error_with_kind(
            CodegenErrorKind::NonPrimitiveType {
                ty: ast_model::print_type(&SolType::Array(array.clone()), declares)
                    .to_string()
                    .into_boxed_str(),
            },
            span,
