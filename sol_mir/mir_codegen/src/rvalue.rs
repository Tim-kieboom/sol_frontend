...
impl<'ctx, 'a> FunctionCodegen<'ctx, 'a> {
    /// Materializes a Sol string literal as a null-terminated LLVM global
    ...
    fn codegen_heap_alloc(
        ...
        let elem_ty = self
            .ctx
            .llvm_type(self.sol_module, self.ctx.resolve_type(ty), None)?;
        ...
        match operand {
            Operand::Copy(place) | Operand::Move(place) if place.projection.is_empty() => {
                matches!(
                    ...
                    SolType::Primitive(p) if p.is_signed()
                )
            }
