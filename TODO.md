...
- [x] Struct name resolution: `DeclareStore.struct_names: VecMap<ModuleId, HashMap<SharedStr, NodeId>>`
      ...
      struct-typed `SolType::Stub`'s bare name back to its `Struct` declaration without re-walking
      ...
      `sol_name_resolver` already had working struct field-type-checking via
...
- [x] Struct field reads/writes/construction lowered through the whole pipeline and proven via real
      exes (`09_struct_field_read.sol`, `10_struct_field_write.sol`, per the "prove one vertical
...
- [x] Array literal construction, `&arr`-to-slice, and slice indexing (read+write), plus bounds
      checking on slice indexing — proven via `12_slice_index.sol` and `13_slice_bounds_check.sol`.
  ...
  - `mir_codegen`: `llvm_type` maps `[N]T` to a real LLVM array type and `[&]T`/`[&mut]T` to a
    `{ptr, len}` struct (`len` at pointer width). `resolve_place`'s walk now tracks the *Sol* type
    ...
    element type after an `Index` step has to come from the `ArrayType` on the Sol side instead.
...
- [x] Overflow checking on arithmetic ops (`+`/`-`/`*`), **at the MIR level** (moved off the original
      codegen-level implementation) — proven via `14_arith_overflow_check.sol` (`i32::MAX + 1`
  ...
  - `mir_model`: new `Rvalue::CheckedBinaryOp(op, left, right)`, producing a `(T, bool)` tuple
    ...
    `SolType::TupleKind(TupleKind::Tuple(..))` — not a new `Rvalue` aggregate-construction case
  ...
  - `step_into_field` (struct-field `Place` resolution) now also accepts a positional-tuple
    `SolType::TupleKind(TupleKind::Tuple(..))` destination alongside a declared struct — needed so
...
- [x] Overflow checking on `/`/`%` (division by zero, and — signed only — `MIN / -1`/`MIN % -1`) —
      proven via `17_div_by_zero_check.sol`, `18_mod_by_zero_check.sol`, `19_div_overflow_check.sol`,
      `21_mod_overflow_check.sol` (all abort with the matching message+location), plus
      `20_div_mod_normal.sol` as a regression guard that ordinary division/remainder still compute
...
- [x] Rust-`panic!`-style panic runtime (message, no backtrace, no unwinding) — every panicking
      ...
      the panic runtime from. Proven via `15_assert_panic_message.sol` and
      `16_panic_intrinsic_message.sol` (stdout matched against `panic: <message>`), plus
      `expect_stdout` added to `13_slice_bounds_check.sol`/`14_arith_overflow_check.sol`.
  - `mir_codegen/src/terminator.rs`: `panic_function` lazily declares *and defines* (once per module)
      a `sol_panic(msg: cstr)` function — `printf("panic: %s\n", msg)`, `fflush(NULL)`, `abort()`,
  ...
  - [x] Panic location (`file:line:col`, à la `thread 'main' panicked at src/main.rs:4:5`) — proven
      ...
      `"<file>.sol:<line>:"`. Still no backtrace (explicitly out of scope, per the user's own
      framing).
    ...
    - `mir_codegen` needed a way to turn a `Span`'s `ModuleId` into an actual file path, which nothing
      ...
      `sol_utils::collections::module_store::ModuleStore` does — the same one
      `sol_tester::display::fault` already uses for its own compile-time diagnostics). Threaded a
    ...
    - `sol_panic`'s signature widened to `(msg: cstr, location: cstr)`, printing
      `"panic: {msg}\n  at {location}\n"`.
...
- [x] Non-generic trait support (static dispatch only, no `Trait<T>`) — proven via
      `26_trait_impl_dispatch.sol` (a real `impl Greeter for Bar` method called through a `Bar`
      value, returning the receiver's own field).
  - `sol_name_resolver`: entirely resolver-level, as scoped — dispatch itself needed no new
  ...
  - Deliberately deferred to a fast-follow (each cuts real scope, not laziness):
    ...
    - A `This` type (Sol's `Self`-equivalent, for a trait method parameter that needs to refer to
...
- [x] Implicit ("trailing") return of a function body's tail expression — both an `=>`
      ...
      `return` and no trailing `;` — proven via `27_trailing_return.sol` (an `=>` body, a
      block-tail, and an exhaustive `if`-tail all feeding into one result).
  - Entirely a `mir_parser` gap: `sol_name_resolver` already type-checked this (`check_tail_
  ...
  - Found and deliberately **not** fixed while proving this (out of scope, pre-existing, unrelated to
    ...
    instead of computing `-4`. `27_trailing_return.sol`'s `neg`/`abs` helpers route every literal
...
- [x] `sol_mir/mir_codegen` — LLVM IR emission via `inkwell` (`features = ["llvm16-0"]`, Windows
      only) implemented for scalar/pointer/struct locals, arithmetic/comparison/logical ops, if/while,
      function calls, `extern "C"` functions (incl. `cstr`/pointer params and correct C-vs-Sol
  ...
  - [x] `f32`/`f64` arithmetic (`+`/`-`/`*`/`/`/`%`) and comparisons (`==`/`!=`/`<`/`>`/`<=`/`>=`) —
        proven via `22_float_arithmetic.sol`, `23_float_ops.sol` (all six ops + all six
        comparisons on `f64`), `24_f32_arithmetic.sol`. `f16` deliberately still unsupported (falls
        through to `UnsupportedPrimitiveType` in `llvm_type` — no direct C ABI use for it yet).
    - `ast_model`/tokenizer/`sol_name_resolver` already had everything needed (`PrimitiveTypes::
...
- [x] Wire codegen output through to an actual `.exe` — `scripts/run_codegen_tests.py` drives
      ...
      is currently the real correctness oracle for the pipeline (`sol_tester/sol/src/codegen_tests/`,
      28 passing exe tests). Still manual/script-driven, not integrated into `cargo test`.
- [x] Audited `sol_name_resolver`'s test coverage of the resolver/typecheck-flavored
      ...
      across `sol_name_resolver`'s test files, then manually cross-checked the misses.
  - 7 of ~34 variants had zero test hits. Of those:
    ...
    - `FunctionNameEmpty`, `FunctionNameInvalidStart`, `VariableNameEmpty`,
      `VariableNameInvalidStart` were **dead code**, not a test gap: `sol_tokenizer`'s
      `lex_ident` (`sol_tokenizer/src/lexer.rs`) is only ever entered on a char already
...
- [x] Fixed a real crash: `x := Struct{field: value}` (an untyped `:=` declaration whose initializer
  ...
  - Root-caused, then generalized: audited every `ExpressionKind` variant `expression_type` was
    ...
    `foreach_collection_element_type`'s own `SolType::Array` unwrap), `Unary` (only `!`/`Not` — `-`
  ...
  - Proven via `28_untyped_struct_constructor.sol` (the exact reported crash, now compiling and
  ...
  - Also added `expression_type` arms for `TypeOf` (split by `TypeofKind`: only `Value`
    ...
    initial pass treated all four as `SolType::Type`, which would have mistyped e.g. `expr typeof
...
- [x] Fixed a real gap: `value: bool = 1` (an explicit `x: T = value` annotation) silently
  ...
  - Surfaced two latent bugs in `combine_resolved_operand_types` once real programs started
    ...
    1. Two `SolType::Array`s only combined via strict structural equality, so `[&mut]T`/`[&]T`
  ...
  - Proven via 4 new `assignment_tests.rs` cases (matching/mismatched explicit declarations, a
    ...
    the latter two were only caught by re-running the full exe suite (`12_slice_index.sol`/
    `13_slice_bounds_check.sol` briefly regressed before the `combine_resolved_operand_types` fix
    above), underscoring why that suite — not just resolver unit tests — is the real oracle here.
- [x] `PlatformInfo` (`sol_utils::compiler_options`) gained a real (if still one-armed)
      ...
      `sol_tester::config::COMPILER_OPTIONS`, and the pipeline benchmark's `COMPILER_OPTIONS` all
  ...
  - Proven via 2 new `sol_utils::compiler_options_tests` cases (`host()` matches
...
- [x] Deref places: `mir_parser` can now lower `*ptr` (explicit) and auto-derefs through a
  ...
  - `require_lowerable` (`function/mod.rs`) gained `SolType::Reference`/`Pointer` — a bare
  ...
  - `mir_codegen::function::resolve_place` gained a `step_into_deref` step: `ptr` holds the
    *address of* the reference's own storage at that point (same invariant every other step
    ...
    actually points at.
  - Also fixed two related bugs in `sol_name_resolver::resolve::typecheck::expression`'s
  ...
  - Proven via `29_deref_places.sol` (explicit deref read+write, plus field access through a
...
- [x] Fix `&this`/`&mut this` receiver lowering to actually pass a reference instead of a
  ...
  - Proven via `30_mut_receiver.sol` (a `&mut this` method mutating a field across two calls,
...
- [x] `Move`/`MarkMoved` lowering for straight-line code (function-call arguments,
  ...
  - [x] `is_auto_copy(SolType)` classification. `/grill-me`'d once the user's own `SolType`
        `TypeId`-flattening refactor landed (nested fields like `ReferenceType.inner`/
        `ArrayType.of_type` are now interned `TypeId`s, not inline `Box<SolType>`), to re-derive
    ...
    - `fn is_auto_copy(&self, ty: &SolType, span: Span) -> MirResult<bool>` (`function/mod.rs`,
      ...
      `SolType` grows new variants. Fallible (returns the same `NonPrimitiveType` error
...
- [ ] Implement borrow checker as a MIR pass over the concrete (M1) subset. `/grill-me`'d "what's
  ...
  - [x] **Wired into the lowerer's own drop-chain decisions** — a real `elaborate_drops` pass
    ...
    - Proven via 2 new/rewritten `mir_parser` unit tests exercising `elaborate_drops` directly
      ...
      tests) and all 33 exe tests — including `32_free_at_drop.sol`/`33_owning_parameter_dropped.
      sol`, which specifically exercise the old Slice 3b/3c mechanism this replaced — pass unchanged.
...
- [ ] **Pipeline architecture cleanup** (`/grill-me`'d 2026-09-17, paused the borrow-checker's own
  ...
  - A full-pipeline scan (beyond the flow-insensitive audit below) also found a second, related
    ...
    `resolve_struct`"*.
  - [x] **Part A1** — pure, `DeclareStore`-independent classifiers moved to `ast_model`/`sol_utils`
        as inherent methods (no caching needed, lowest risk — cheap to recompute, callers just no
        longer duplicate the rule):
    - `PrimitiveTypes::{is_signed, is_float, signed_min}` (`sol_utils::sol_names`) — absorbs
    ...
    - `SolType::is_primitive` (`ast_model::ast::sol_type`) — the pure shape-check half of
      ...
      classification instead of re-matching `SolType::Primitive(_)` inline.
  ...
  - [x] **Part A2** — moved `resolve_struct`/`is_lowerable`/`is_auto_copy` (formerly
    ...
    - **No cache** — dropped from the original plan after a `/grill-me` follow-up surfaced a real
      correctness hazard: `SolType::Stub` only carries a bare `name` (no module qualifier), so two
      *different* modules' same-named-but-different structs intern to the **same** `TypeId` — a
      ...
      - **Smaller stopgap (not done):** give `Stub` an `Option<ModuleId>` field that participates in
        ...
        production construction sites (`ast_parser`'s `parse/sol_type.rs` ×2,
      ...
      - **Bigger, root-cause redesign (not done):** stop interning bare-name `Stub` references into
        the long-lived `SolType`/`TypeId` system at all — have the resolver replace a `Stub` with a
        real resolved reference (e.g. `SolType::Struct(StructId)`, pointing straight at the
  ...
  - [x] **Part A3** — moved `is_type_boolean`/`expression_is_bool` (`mir_parser::function::type`)
        ...
        `is_variable_boolean` helper) — not literally "into the `sol_name_resolver` crate" as
        originally worded: `mir_parser` only depends on `ast_model`/`mir_model`/`sol_utils` in
        production code (`sol_name_resolver` is a dev-dependency, tests only), so anything
  ...
  - [x] **Part A4** — new `SolType::deref_once(&self, declares) -> Option<SolType>`
        (`ast_model::ast::sol_type`): the shared type decision (is this `&T`/`&mut T`/`*T`, what's
    ...
    - `sol_name_resolver`'s `expression_type`'s `Deref` arm and `struct_field_type`'s auto-deref
    ...
    - Proven by the same full `cargo test --workspace` (zero warnings) + all 33 exe tests bar as
      every A-slice above — pure relocation, no behavior change. `29_deref_places.sol` specifically
  ...
  - [x] **Part A5** — partial, as scoped, plus a correction found while doing it: the function
        ...
        `step_into_field`/`step_into_index`/`step_into_deref`, which compute the same Sol-type
        transition interleaved with the LLVM pointer/GEP mechanics.
    - New `PlaceElem::step_type(&self, ty, declares, module) -> Option<SolType>` (`mir_model`, not
      ...
      `Deref` arm was *also* still duplicating `SolType::deref_once` (A4) inline — folded in too,
      a gap the earlier A4 pass missed.
    - `mir_codegen::function::resolve_place`'s three steps: `step_into_deref` now calls
      `sol_ty.deref_once(declares)` directly (a clean full swap — no new `mir_model` code needed,
  ...
  - [x] **Part C** — `operand_is_signed` was already retired in A1; `mir_codegen::rvalue::operand_type`
    ...
    - `mir_codegen::types::resolve_struct` — its only remaining call site (`stub_type`, within the
      ...
      deleted.
    - New `ArrayKind::is_lowerable(self) -> bool` (`ast_model::ast::sol_type`) — the actual shared
  ...
  - [x] A and C landed, B confirmed unnecessary — resumed the paused borrow-checker slice. See the
        ...
        for why).
- [x] **Replace `SolType::Stub` with resolved type references** — the "bigger, root-cause redesign"
      ...
      just structs. Mechanism settled on: **not** AST mutation (`sol_name_resolver`'s `NameResolver`
  ...
  - [x] **Slice 1 — identity only.** `Stub` gained `occurrence: NodeId` (`ast_model::ast::
        sol_type`), participating in its own `Eq`/`Hash` — a fresh id allocated once per syntactic
        ...
        occurrence)` is the real constructor (used at all 4 production sites — `parse/sol_type.rs`
    ...
    - **The correctness risk the plan called out as the crux of this slice, found and fixed**: any
      comparison relying on "same `TypeId`/`SolType` ⇒ same type" for a `Stub`-shaped value now
      ...
      `DeclareStore::{types_equal_ignoring_occurrence, sol_types_equal_ignoring_occurrence}` — a
      recursive structural-equality check that treats two `Stub`s as equal whenever `name`/
      `generics` match, ignoring `occurrence`, recursing through every `TypeId`-nesting `SolType`
    ...
    - Three real call sites found and fixed (an Explore audit pass, cross-checked by hand):
      `combine_resolved_operand_types` (`sol_name_resolver`'s single choke point for
      ...
      suite: it broke 3 `mir_parser` receiver tests and 1 `sol_name_resolver` ambiguous-impl test
    ...
    - New regression test proving the fix actually works (per the plan's own verification bar):
      ...
      (`sol_name_resolver`'s `function_call_argument_tests.rs`) — a struct named in 3 separate
  ...
  - [x] **Slice 4** — `check_impl_conformance` (`resolve/statement.rs`) now populates
        ...
        warnings) + all 33 exe tests bar as every slice above (`26_trait_impl_dispatch.sol`
  ...
  - [x] **Slice 7** — same fork as Slice 2, resolved the same way: the plan's original design ("walk
    ...
    - Proven by the same full `cargo test --workspace` (zero warnings, 138 `sol_name_resolver`
...
- [x] `new(expr)` heap allocation (`*T`, an owning pointer — Rust's `Box<T>`), motivated by the
  ...
  - [x] **Slice 1** — prerequisite correctness fixes, landed first since they were needed
    ...
    - Corrected 3 existing (unimplemented, forward-declared-only) intrinsics' doc comments —
      ...
      same `SolType`, indistinguishable to `is_auto_copy`/`Drop`.
    - `is_auto_copy` no longer treats `SolType::Pointer` (`*T`) as `AutoCopy` — corrected to
      move-only, same as a struct. `&T`/`&mut T` (`SolType::Reference`) is unaffected, still
  ...
  - [x] **Slice 2** — `new(expr)` allocates via `malloc` and produces a usable `*T`. Deliberately
    ...
    - New `mir_model::Rvalue::HeapAlloc(Type, Operand)` — carries the element type explicitly
      ...
      unlike a real Sol function call there's nothing to branch on, so it doesn't need `Call`'s
      terminator shape.
    ...
    - Proven via a new `mir_parser` unit test (`new_expression_lowers_to_a_heap_alloc_rvalue`), a
      ...
      in_a_binary_expression`), and a new exe test (`31_new_expression.sol`: `new(9)` then `*p`
  ...
  - [x] **Slice 3b** — `Drop` actually calls `free()` for a `*T`, gated entirely at *compile time*
        ...
        (`SolType::Pointer`) loads the pointer and calls `free()` (declared lazily via the same
        ...
        (`32_free_at_drop.sol`: a `new(7)` pointer that's read via `*p` but never moved or
    ...
    - Found and fixed a real bug while writing that test: `new(1)`'s bare literal argument was
      ...
      expected). This also surfaced that `31_new_expression.sol`/`32_free_at_drop.sol` had been
  ...
  - [x] **Slice 3c** — fixed a real leak: an owning parameter (or by-value `this`) was never
        ...
        (`33_owning_parameter_dropped.sol`); full workspace suite and all 33 exe tests pass.
...
- [x] `extern "C"` variadic functions (`name: varargs` as an extern's last parameter, called as
      ...
      `varargs` stub) — `sol_name_resolver`'s `finish_call_resolution`/`check_varargs_argument`
      ...
      `codegen_variadic_argument`/`variadic_operand_sol_type`, purely at the LLVM-operand level —
      never a user-visible cast. Proven via real exes: `34_extern_variadic_printf_fixed.sol`,
      ...
      - Prerequisite bug fixed along the way: `declare_all_functions` hardcoded `is_var_arg = false`
        ...
        `is_variadic` through as above. Note this only fixes a call declared *as* `varargs` in Sol —
        a fixed-arity Sol declaration of a function that happens to be variadic in the real C
        library (e.g. `printf(format: cstr, arg: f64)` with no `varargs` marker) is unfixable in
        principle: Sol has no way to know the real callee is variadic unless told via `varargs`, so
...
- [ ] `TypeId` interning for `any` (a runtime `void*` + `TypeId` payload) and `var.typeof == int`
  ...
  - [x] Foundation: `TypeId` (`impl_sol_ids!(TypeId)` in `ast_model/src/ast/sol_type.rs`, a plain
        monotonic `usize` like every other id type) plus `DeclareStore::{intern_type, get_type,
        get_type_id}` wrapping the existing `sol_utils::collections::bimap::BiMap<TypeId,
        SolType>` (this collection already existed — no new interning mechanism had to be built).
        `SolType` and its nested types (`TupleKind`, `ArrayType`, `ReferenceType`, `Stub`) gained
        `Eq`/`Hash` (plus `Mutable` in `sol_utils`) so they can key the `BiMap`; no other behavior
  ...
  - [x] Comptime `TypeId` constants for every context-free (parameterless) `SolType` —
        ...
        `impl TypeId` in `sol_type.rs`. `DeclareStore::new` interns all 35 in that exact order via
        ...
        of `declares.get_type(signature.return_type) == Some(&SolType::None)` — applied at the 3
        `mir_parser`/`DeclareStore`/`sol_name_resolver` call sites that were doing exactly that
        (`FunctionLowerer::lower`'s/`lower_extern_signature`'s/the call-lowering in
        `function/statement.rs`'s none-return checks skip resolving the `SolType` entirely now
        ...
        their `SolType`; interning a fresh equivalent `SolType` returns the same constant) plus the
  ...
  - [x] `ast_parser::Parser` threaded a `&mut DeclareStore` (new `ParseInfo.declares` field, plumbed
        ...
        `TypeId` from the moment it exists, never a transient `SolType`.
  - [x] First real field converted end-to-end: `Parameter.ty` (`SolType` -> `TypeId`) —
        `ast_parser`'s 3 construction sites now call `self.intern_type(..)`; every consumer
        (`sol_name_resolver::collect::statement`'s `collect_extern_signature`/`collect_function`,
        `sol_name_resolver::resolve::typecheck::function_call`'s argument-type checking,
        `mir_parser`'s `FunctionLowerer::lower` and `lower_extern_signature`, `sol_tester`'s AST
        display) now resolves the `TypeId` back to `&SolType` via `declares.get_type` before using
  ...
  - [x] `Variable.ty` (`Option<SolType>` -> `Option<TypeId>`) — this is the one field shared by
        ...
        `try_parse_from_mut` interning sites; `sol_name_resolver`'s `collect_variable` (backfill vs.
        ...
        LLVM struct-type construction); `sol_tester`'s AST display for both a plain variable's type
        ...
        read.sol`/`10_struct_field_write.sol`/`11_nested_struct_field.sol` and others.
  - [x] `InnerFunctionSignature.{method_type, return_type}` (`SolType` -> `TypeId`, both always
        present — no `Option`, `SolType::None` is its own canonical interned entry for "no method
        ...
        of comparing `SolType` directly; `sol_name_resolver`'s `collect_extern_signature`/
        ...
        all resolve via `self.declares`/an explicit `declares` parameter; `sol_tester`'s AST display.
  ...
  - [x] `TypeDef.{new_type, old_type}` (`SolType` -> `TypeId`, `type X := Y`/`type X := distinct Y`)
        — smaller, well-contained surface: `ast_parser`'s single `parse_typedef` interning site;
        `sol_name_resolver`'s `collect_statement` (both types resolved once up front so the
        `!is_distinct` alias-registration branch, which still stores a real `SolType` in
        `DeclareStore::type_aliases`, doesn't need a second lookup); `sol_tester`'s AST display.
        Proven by the same full `cargo test --workspace` + 30-exe-test bar as the fields above.
  - [x] `UseBlock.ty` (`SolType` -> `TypeId`, `use Foo { .. }`'s own type). 3 `ast_parser`
        construction sites (`parse_use_block`'s two exit paths, `parse_impl_statement`) now intern;
        `sol_name_resolver`'s `collect_use_block` and `sol_tester`'s AST display resolve back.
        `ImplBlock.impl_trait` (also read by `collect_use_block`/`check_impl_conformance`) is a
        separate, still-`SolType` field — deliberately not touched here. Proven by the same full
        `cargo test --workspace` + 30-exe-test bar as the fields above.
  - [x] `ImplBlock.impl_trait` (`SolType` -> `TypeId`, `impl Trait { .. }`'s target). Single
        `ast_parser` interning site (`parse_impl_block`, shared by both its early-return and
        curly-brace exit paths); `sol_name_resolver`'s `check_impl_conformance` (the
        `let SolType::Stub(stub) = &impl_block.impl_trait else { .. }` pattern became a `let
        Some(SolType::Stub(stub)) = self.declares.get_type(..) else { .. }`, same shape) and
        `collect_use_block`; `sol_tester`'s `write_impl`. Proven by the same full
        `cargo test --workspace` + 30-exe-test bar as the fields above (`26_trait_impl_dispatch.sol`
        specifically exercises this field at runtime).
  - [x] `Enum.impl_type` (`Option<SolType>` -> `Option<TypeId>`, `enum Foo: int { .. }`'s backing
        type). The smallest surface yet — no `sol_name_resolver`/`mir_parser` consumer reads this
        ...
        Only `ast_parser`'s `parse_enum` interning site and `sol_tester`'s `write_enum` needed
  ...
  - [x] `Trait.typedefs` (`Vec<SolType>` -> `Vec<TypeId>`, `trait Foo { type Bar; .. }`'s associated
        type declarations) and `UnionKind::{Tuple, NamedTuple}`'s `parameters` (`Vec<SolType>`/
        `Vec<(Ident, SolType)>` -> `Vec<TypeId>`/`Vec<(Ident, TypeId)>`, a union/enum variant's
        ...
        `sol_tester`'s enum-variant display. `ast_parser`'s 3 interning sites: `parse_trait`'s
  ...
  - [x] `StructConstructor.struct_type` (`Point{x:1}`'s own target type) and `Array`/`ArrayFiller`'s
        ...
        `parse_array`/`parse_array_filler`/`parse_array_literal`); `sol_name_resolver`'s
        `collect_struct_constructor`/`collect_any_array`, `check_struct_constructor` (the
        `let SolType::Stub(stub) = &struct_constructor.struct_type` pattern became `let Some(
        SolType::Stub(stub)) = self.declares.get_type(..)`, same shape as `check_impl_conformance`
        ...
        `struct_type` once up front, same pattern as `Parameter.ty`); `sol_tester`'s
        ...
        `25_ctor_and_methode.sol`/`28_untyped_struct_constructor.sol` exercise `struct_type`
        ...
        This closes out every AST-node-attached `SolType` field identified when this series started
        — `SolType`'s own internal recursive fields (`ArrayType.of_type`, `Reference.inner`,
        `Optional`'s payload, etc.) remained intentionally un-interned at this point, per the scope
        decision at the top of this entry.
  - [x] The full flatten, reversing the scope decision above: every `SolType`-internal recursive
        ...
        `TupleKind`'s `Tuple`/`NamedTuple` element types) is `TypeId` now, not `Box<SolType>`/
        `SolType`. Also swept up two AST-node fields missed by the earlier series while here:
        `FunctionCall.generics` and the (never-actually-constructed) `FunctionCalleeKind::Type`.
    - The blocker flagged before starting — `SolType`'s `Debug` impl recursing through owned
      `Box<SolType>` fields with no way to resolve a bare `TypeId` — is real and was resolved by
      ...
      DeclareStore` alongside the `&SolType`, resolving each nested `TypeId` as it recurses. Every
      site that used to do `format!("{ty:?}")` for a user-facing error/fault message — ~30 of them
      across `sol_name_resolver`'s typecheck modules, `mir_parser`, `mir_codegen` — now goes through
      a small `print_ty(&self, ty: &SolType) -> String` helper (one per crate that needs it) wrapping
      ...
      tests (`sol_name_resolver`'s `lambda_type_tests`, checking `got.contains("-> int")`) caught it
      here (7 tests failed on exactly this before the sweep — grep for `format!("{` + a `:?}` type
      specifier is what actually found the rest, not the compiler). One further, non-`SolType`
      instance of the same shape: `EnumVariantArgumentTypeMismatch` stored raw `SolType` fields and
    ...
    - Every one of the ~30 direct `SolType`-recursive-variant construction sites across
      `ast_parser`'s single type-parsing module (`parse/sol_type.rs`) now interns as it builds
    ...
    - `mir_parser::FunctionLowerer.declares` had to become `&'a mut DeclareStore` (previously `&'a
        DeclareStore`, since this crate — unlike `sol_name_resolver` — only ever *read* through the
        interner before): lowering a `&this`/`&mut this` receiver or a checked-arithmetic op's
        `(T, bool)` result tuple builds a brand-new `SolType` on the fly that has to be interned to
        ...
        `mir_run::to_mir`, `sol_tester::main`'s own `fn mir`.
    - In `sol_name_resolver`, several previously-`&self` methods that build a fresh nested
        `SolType` needed to become `&mut self` for the same reason (`expression_type`,
        ...
        a later mutable call, unlike the read-only `mir_parser`/`mir_codegen`/`sol_tester` crates.
    - Two comptime `TypeId` constants got real use here for the first time: `literal_type`'s
        `Literal::Str` arm and a `RawPtr<int>`/`Res<int, str>` parser test both build a `SolType`
        value using `TypeId::STRING`/`TypeId::PRIM_INT` directly instead of interning a fresh
        `SolType::String`/`SolType::Primitive(Int)` — the exact use case the constants exist for.
