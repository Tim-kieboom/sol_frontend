...
fn expression_type_of_binding(ast: &AstTree, name: &str) -> Option<SolType> {
...
fn two_untyped_int_literals_combine_and_stay_untyped() {
    ...
    assert_eq!(
        expression_type_of_binding(&ast, "c"),
        Some(SolType::Primitive(PrimitiveTypes::UntypedInt))
    );
}
...
fn untyped_chain_with_float_stays_untyped_float() {
    ...
    assert_eq!(
        expression_type_of_binding(&ast, "c"),
        Some(SolType::Primitive(PrimitiveTypes::UntypedFloat))
    );
}
