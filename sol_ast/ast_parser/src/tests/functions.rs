...
fn function_with_multi_where_clause() {
    ...
    match bound.as_ref().unwrap() {
        SolType::Stub(stub) => assert!(stub.matches_ignoring_occurrence(&Stub::new("A"))),
        other => panic!("expected a Stub named A, got {other:?}"),
    }
    let Generic { name, bound } = &signature.value.generics[1];
    assert_eq!(name.as_str(), "U");
    match bound.as_ref().unwrap() {
        SolType::Stub(stub) => assert!(stub.matches_ignoring_occurrence(&Stub::new("B"))),
...
fn param_impl_trait() {
    ...
    match declares.get_type(signature.value.parameters[0].ty) {
        Some(SolType::ImplTrait(id)) => match declares.get_type(*id) {
