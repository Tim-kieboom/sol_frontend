...
use sol_tokenizer::model::{StringFormatTag, TokenKind, keyword::KeyWord};
use sol_utils::{
    Ident, TypeModifier,
        ...
        loop {
            match &self.token().kind {
                TokenKind::FStringPart(text) => {
                    ...
                    self.parse_fstring_part(&mut parts, &mut trailing, text)?;
                }
    ...
    pub(super) fn parse_primary_ident(
        ...
        match &self.token().kind {
            &COLON if peek.kind == SQUARE_OPEN => {
                return Err(
                    sol_error_internal!("collectionType array not yet impl", Some(span))
                        .into_kind(),
                );
            }
