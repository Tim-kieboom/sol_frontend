use ast_model::{
    Enum, EnumVariant, Field, SolType, Statement, StatementKind, Struct, Stub, UnionKind,
};
use sol_tokenizer::model::{TokenKind, keyword::KeyWord};
use sol_utils::{Ident, collections::try_result::ToResult, fault::Fault};
...
impl<'a, 'f> Parser<'a, 'f> {
    pub fn parse_struct(&mut self) -> AstResult<Statement> {
        ...
        let this_type = SolType::Stub(Stub::new_at(struct_name.to_string(), self.alloc_node()));
