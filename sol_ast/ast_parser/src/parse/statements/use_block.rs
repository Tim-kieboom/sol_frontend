use ast_model::{ImplBlock, Methode, SolType, Statement, StatementKind, UseBlock};
...
impl<'a, 'f> Parser<'a, 'f> {
    ...
    pub(crate) fn parse_impl_statement(&mut self, start_span: Span) -> AstResult<Statement> {
        let method_type = self.current.this_type.clone().unwrap_or(SolType::None);
    ...
    fn parse_use_method(&mut self, ty: &SolType, start_span: Span) -> AstResult<Methode> {
