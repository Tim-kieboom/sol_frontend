use ast_model::{
    Expression, ExpressionKind, FunctionKind, FunctionSignature, FunctionSignatureHelper, SolType,
...
impl<'a, 'f> Parser<'a, 'f> {
    ...
    fn parse_trait(&mut self) -> AstResult<Statement> {
        ...
        let this_type = SolType::Stub(Stub::new_at(name.as_str(), self.alloc_node()));
        loop {
            self.skip_end_lines();
