...
impl<'a, 'f> Parser<'a, 'f> {
            ...
            self.log_fault(
                sol_error_internal!(
                    format!(
                     ...
                     use linkage = \"static\" in Sol.toml"
                    ),
                    Some(span)
                )
                .into_kind(),
    ...
    fn import_module(
        ...
        let Some(folder_path) = module_file_path.parent() else {
            self.log_fault(
                sol_error_internal!("module_file_path should have parent", None).into_kind(),
            );
            return ModuleId::ERROR;
        };
    ...
    fn find_crate_root_file(
        ...
        for filename in ["lib.sol", "main.sol", "mod.sol"] {
    ...
    fn insure_parents_are_loaded(
        ...
        let relative_path = match module_file_path.strip_prefix(base_path) {
            ...
            Err(err) => {
                self.log_fault(
                    sol_error_internal!(format!("{}", err.to_string()), None).into_kind(),
                );
                return;
            }
        };
