...
impl FuncError {
    pub fn new(ident: Ident, fault: AstFault) -> Box<Self> {
        Box::new(Self { ident, fault })
    }
}

type FuncResult<T, K = AstFault> = TryResult<T, Box<FuncError>, K>;
impl<'a, 'f> Parser<'a, 'f> {
    pub(crate) fn parse_any_function(&mut self) -> AstResult<Statement> {
        ...
        let this_type = self.current.this_type.take().unwrap_or(SolType::None);
    ...
    pub(crate) fn try_parse_function_call(
        ...
        if self.current_is(&DOT) {
            ...
            return TryErr(Fault::error_with_kind(
                crate::fault::AstErrorKind::ExpectedArrayLiteralOrParenAfterTypeConstructor,
    ...
    pub(crate) fn try_parse_function_declaration_id(
        &mut self,
        start_span: Span,
        methode_type: &SolType,
        is_const: bool,
        name: Ident,
    ) -> FuncResult<Spanned<FunctionId>, crate::fault::AstFault> {
    ...
    pub(crate) fn try_parse_function_declaration(
        ...
        is_const: bool,
        name: Ident,
    ) -> FuncResult<Spanned<Function>, AstFault> {
    ...
    ) -> FuncResult<FunctionSignature, AstFault> {
    ...
    pub(crate) fn parse_generic_declare(&mut self) -> AstResult<Option<Vec<Generic>>> {
    ...
    fn parse_where_clause(&mut self, mut generics: Vec<Generic>) -> AstResult<Vec<Generic>> {
    ...
    fn inner_function_declaration(
        ...
        is_const: bool,
        external: Option<ExternLanguage>,
    ) -> FuncResult<Spanned<Function>, crate::fault::AstFault> {
        let signature =
    ...
    fn inner_parameters(
        ...
        loop {
            ...
            if self.current_is_ident(VARARGS_IDENT) && next_ends_param {
                ...
                let ty = self.intern_type(SolType::None);
