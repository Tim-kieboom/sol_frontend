use crate::{
    ArrayKind, ArrayType, NodeId, SolType, Struct, Stub, TypeId,
...
fn interning_the_same_type_twice_returns_the_same_id() {
    ...
    let int_ty = SolType::Primitive(PrimitiveTypes::Int);
...
fn different_types_get_different_ids() {
    ...
    let bool_id = declares.intern_type(SolType::Primitive(PrimitiveTypes::Boolean));

    assert_ne!(int_id, bool_id);
}

#[test]
fn get_type_resolves_an_interned_id_back_to_its_canonical_sol_type() {
    let mut declares = DeclareStore::new();
    let int_ty = SolType::Primitive(PrimitiveTypes::Int);
...
fn get_type_id_does_not_intern_an_unseen_type() {
    ...
    let unseen_ty = SolType::Stub(Stub::new("SomeStructNeverInterned"));
...
fn well_known_type_ids_are_preinterned_by_new() {
    ...
    assert_eq!(declares.get_type(TypeId::NONE), Some(&SolType::None));
    assert_eq!(declares.get_type(TypeId::ERROR_TYPE), Some(&SolType::Error));
    assert_eq!(
...
fn interning_a_well_known_type_again_returns_its_comptime_constant() {
    ...
    // fresh SolType and interning it — never by naming the TypeId constant
    ...
    let id = declares.intern_type(SolType::Primitive(PrimitiveTypes::Int));
    assert_eq!(id, TypeId::PRIM_INT);

    let id = declares.intern_type(SolType::None);
...
fn type_resolve_round_trips_through_insert_and_get() {
    let mut declares = DeclareStore::new();
    let occurrence = declares.intern_type(SolType::Stub(Stub::new("Point")));
