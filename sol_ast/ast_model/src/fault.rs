...
pub enum AstErrorKind {
    ...
    #[error("external crate '{lib_name}' not found in Sol.toml dependencies")]
    ExternalCrateNotFound { lib_name: Box<str> },

    #[error("no 'mod.sol' found in folder '{path:?}'")]
    ...
    #[error(
        "crate '{crate_name}' has no root file (lib.sol, main.sol, or mod.sol) in '{source_root}'"
    )]
    ...
    // ----------------------------------------------------------------
    //  Name resolution (sol_name_resolver shares this CrateContext)
    // ----------------------------------------------------------------
    ...
    #[error(transparent)]
    LexError(#[from] sol_tokenizer::fault::TokenErrorKind),
}
...
pub struct EnumVariantArgumentTypeMismatch {
    ...
    /// `SolType` — unlike a boundary/leaf `TypeId`, `SolType`'s own
