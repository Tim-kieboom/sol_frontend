...
use crate::{
    CustomType, Enum, ExpressionId, InnerFunctionSignature, NodeId, SolType, Struct, Trait, TypeId,
};
use sol_utils::{
...
pub struct DeclareStore {
    ...
    /// (MIR lowering) resolve a struct-typed `SolType::Stub`'s bare name
    ...
    variable_type: VecMap<NodeId, (TypeModifier, Option<SolType>, ModuleId)>,
    /// Resolved type of an expression, indexed by its ID.
    expression_types: VecMap<ExpressionId, SolType>,
    ...
    type_aliases: HashMap<SharedStr, SolType>,
    /// Canonical `TypeId <-> SolType` interning table — see [`Self::intern_type`].
    type_ids: BiMap<TypeId, SolType>,
...
impl DeclareStore {
    ...
    /// Interns every context-free `SolType` in the exact order `TypeId`'s
    ...
    fn register_well_known_types(&mut self) {
        use sol_utils::sol_names::PrimitiveTypes as Prim;
        ...
        register!(
            TypeId::NONE =>                 SolType::None,
            TypeId::NEVER =>                SolType::Never,
            TypeId::STRING =>               SolType::String,
            TypeId::FORMAT_STRING =>        SolType::FormatString,

            TypeId::ANY =>                  SolType::Any,
            TypeId::TYPE =>                 SolType::Type,
            TypeId::ERROR_TYPE =>           SolType::Error,

            TypeId::PRIM_CHAR =>            SolType::Primitive(Prim::Char),
            TypeId::PRIM_CHAR8 =>           SolType::Primitive(Prim::Char8),
            TypeId::PRIM_CHAR16 =>          SolType::Primitive(Prim::Char16),
            TypeId::PRIM_CHAR32 =>          SolType::Primitive(Prim::Char32),
            TypeId::PRIM_CHAR64 =>          SolType::Primitive(Prim::Char64),

            TypeId::PRIM_CSTR =>            SolType::Primitive(Prim::CStr),
            TypeId::PRIM_NONE =>            SolType::Primitive(Prim::None),
            TypeId::PRIM_BOOLEAN =>         SolType::Primitive(Prim::Boolean),
            TypeId::PRIM_CINT =>            SolType::Primitive(Prim::CInt),

            TypeId::PRIM_UNTYPED_INT =>     SolType::Primitive(Prim::UntypedInt),
            TypeId::PRIM_INT =>             SolType::Primitive(Prim::Int),
            TypeId::PRIM_INT8 =>            SolType::Primitive(Prim::Int8),
            TypeId::PRIM_INT16 =>           SolType::Primitive(Prim::Int16),
            TypeId::PRIM_INT32 =>           SolType::Primitive(Prim::Int32),
            TypeId::PRIM_INT64 =>           SolType::Primitive(Prim::Int64),
            TypeId::PRIM_INT128 =>          SolType::Primitive(Prim::Int128),
            TypeId::PRIM_CUINT =>           SolType::Primitive(Prim::CUint),

            TypeId::PRIM_UNTYPED_UINT =>    SolType::Primitive(Prim::UntypedUint),
            TypeId::PRIM_UINT =>            SolType::Primitive(Prim::Uint),
            TypeId::PRIM_UINT8 =>           SolType::Primitive(Prim::Uint8),
            TypeId::PRIM_UINT16 =>          SolType::Primitive(Prim::Uint16),
            TypeId::PRIM_UINT32 =>          SolType::Primitive(Prim::Uint32),
            TypeId::PRIM_UINT64 =>          SolType::Primitive(Prim::Uint64),
            TypeId::PRIM_UINT128 =>         SolType::Primitive(Prim::Uint128),

            TypeId::PRIM_UNTYPED_FLOAT =>   SolType::Primitive(Prim::UntypedFloat),
            TypeId::PRIM_FLOAT16 =>         SolType::Primitive(Prim::Float16),
            TypeId::PRIM_FLOAT32 =>         SolType::Primitive(Prim::Float32),
            TypeId::PRIM_FLOAT64 =>         SolType::Primitive(Prim::Float64),
        );
    }
    ...
    /// `FunctionId` is.
    pub fn find_function(&self, name: &str, owner_type: Option<&SolType>) -> FunctionLookup {
            ...
            let is_match = match owner_type {
                Some(owner) => self
                    ...
                    .is_some_and(|method_type| {
                        self.sol_types_equal_ignoring_occurrence(method_type, owner)
                    }),
    ...
    /// `SolType::Stub` that a struct-typed value carries) back to its
    ...
    /// Resolves a struct-typed `SolType::Stub`'s bare name back to its
    ...
    /// is unsound any more.
    pub fn resolve_struct(&self, ty: &SolType, module: Option<ModuleId>) -> Option<&Struct> {
    ...
    pub fn is_lowerable(&self, ty: &SolType, module: Option<ModuleId>) -> bool {
        let is_lowerable_array = matches!(ty, SolType::Array(array) if array.kind.is_lowerable());
        matches!(
            ty,
            SolType::Primitive(_) | SolType::Reference(_) | SolType::Pointer(_)
    ...
    /// it's a `SolType::Array`.
    ///
    /// `SolType::Pointer` (`*T`) is deliberately **not** `AutoCopy` — unlike
    ...
    pub fn is_auto_copy(&self, ty: &SolType) -> bool {
        match ty {
            SolType::Primitive(_) | SolType::Reference(_) => true,
            SolType::Array(array) => {
                matches!(
                    array.kind,
    ...
    pub fn types_equal_ignoring_occurrence(&self, a: TypeId, b: TypeId) -> bool {
        ...
        match (self.get_type(a), self.get_type(b)) {
            (Some(ta), Some(tb)) => self.sol_types_equal_ignoring_occurrence(ta, tb),
            _ => false,
        }
    }

    pub fn sol_types_equal_ignoring_occurrence(&self, a: &SolType, b: &SolType) -> bool {
        ...
        match (a, b) {
            (SolType::Stub(sa), SolType::Stub(sb)) => {
                sa.name == sb.name
            ...
            (SolType::Array(aa), SolType::Array(ab)) => {
                aa.kind == ab.kind && ids_eq(aa.of_type, ab.of_type)
            }
            (SolType::Reference(ra), SolType::Reference(rb))
            | (SolType::Pointer(ra), SolType::Pointer(rb)) => {
                ra.mutable == rb.mutable && ids_eq(ra.inner, rb.inner)
            }
            (SolType::RawPtr(pa), SolType::RawPtr(pb)) => opt_ids_eq(*pa, *pb),
            (SolType::Res { ok: oa, err: ea }, SolType::Res { ok: ob, err: eb }) => {
                opt_ids_eq(*oa, *ob) && opt_ids_eq(*ea, *eb)
            }
            (SolType::Optional(pa), SolType::Optional(pb))
            | (SolType::ImplTrait(pa), SolType::ImplTrait(pb)) => ids_eq(*pa, *pb),
            (
                SolType::NamedVariant {
                    base: ba,
                    variant: va,
                },
                SolType::NamedVariant {
            ...
            (SolType::TupleKind(ta), SolType::TupleKind(tb)) => match (ta, tb) {
    ...
    pub fn get_variable_type(
        &self,
        index: NodeId,
    ) -> Option<&(TypeModifier, Option<SolType>, ModuleId)> {
    ...
    pub fn get_expression_type(&self, index: ExpressionId) -> Option<&SolType> {
        self.expression_types.get(index)
    }

    /// Sets the resolved type of an expression.
    pub fn insert_expression_type(&mut self, index: ExpressionId, ty: SolType) {
        self.expression_types.insert(index, ty);
    }

    /// Registers a non-`distinct` `type X := Y` alias's underlying type.
    pub fn insert_type_alias(&mut self, name: impl Into<SharedStr>, underlying: SolType) {
        self.type_aliases.insert(name.into(), underlying);
    }

    /// The underlying type of a non-`distinct` alias by name, if `name` is one.
    pub fn get_type_alias(&self, name: &str) -> Option<&SolType> {
        self.type_aliases.get(name)
    }

    /// Interns `ty`, returning its canonical `TypeId`. Interning the same
    /// `SolType` value twice (by `PartialEq`) always returns the same id —
    /// this is the single entry point that keeps the `TypeId <-> SolType`
    /// mapping 1:1, so callers should never construct a `TypeId` any other way.
    pub fn intern_type(&mut self, ty: SolType) -> TypeId {
        self.type_ids.insert(&mut self.type_id_alloc, ty)
    }

    /// Looks up the canonical `SolType` a `TypeId` was interned from.
    pub fn get_type(&self, id: TypeId) -> Option<&SolType> {
        self.type_ids.get_value(id)
    }

    /// Looks up the `TypeId` `ty` was interned as, without interning it if
    /// it hasn't been seen before.
    pub fn get_type_id(&self, ty: &SolType) -> Option<TypeId> {
