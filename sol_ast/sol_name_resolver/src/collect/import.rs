...
impl<'a> NameResolver<'a> {
    pub(super) fn collect_import_path(&mut self, path: &ImportPath, span: Span) {
        let module_name = match path.module.get_module_name() {
            Some(val) => val,
            None => {
                self.log_fault(sol_error_internal!("could not get module name", None));
                return;
            }
        };
    ...
    fn collect_internal_module(&mut self, path: &ImportPath, span: Span) -> ModuleId {
        ...
        let result = self
            .modules
            .get_id(pathbuf)
            .or_else(|| self.modules.get_id(&pathbuf.with_extension("sol")))
            .or_else(|| self.modules.get_id(&pathbuf.join("mod.sol")));
    ...
    fn collect_external_module(&mut self, path: &ImportPath, span: Span) -> ModuleId {
        ...
        let result = if module_path.as_os_str().is_empty() {
            ...
            self.modules
                .get_id(&root.join("lib.sol"))
                .or_else(|| self.modules.get_id(&root.join("main.sol")))
                .or_else(|| self.modules.get_id(&root.join("mod.sol")))
        } else {
            let full_path = crate_entry.source_root.join(&module_path);
            self.modules
                .get_id(&full_path)
                .or_else(|| self.modules.get_id(&full_path.with_extension("sol")))
                .or_else(|| self.modules.get_id(&full_path.join("mod.sol")))
        };
