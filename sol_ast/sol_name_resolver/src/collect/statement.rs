use ast_model::{
    ...
    FunctionSignature, FunctionThisKind, Literal, SolType, StatementId, StatementKind, TypeId,
...
impl<'a> NameResolver<'a> {
    pub(super) fn collect_statement(&mut self, id: StatementId) {
        let Some(statement) = self.store.statements.get(id) else {
            self.log_fault(sol_error_internal!(format!("{id:?} not found"), None));
            return;
        };
        ...
        match &statement.node {
            ...
            StatementKind::TypeDef(type_def) => {
                ...
                if !type_def.is_distinct
                    && let Some(SolType::Stub(stub)) = &new_type
    ...
    fn infer_literal_type(&self, initialize_value: Option<ExpressionId>) -> Option<SolType> {
        ...
        Some(match literal {
            Literal::Int(_) | Literal::Uint(_) => SolType::Primitive(PrimitiveTypes::Int),
            Literal::Float(_) => SolType::Primitive(PrimitiveTypes::Float64),
            Literal::Bool(_) => SolType::Primitive(PrimitiveTypes::Boolean),
            Literal::Char(_) => SolType::Primitive(PrimitiveTypes::Char),
            Literal::Str(_) => SolType::String,
