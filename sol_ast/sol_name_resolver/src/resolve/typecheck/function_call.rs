use ast_model::{
    ...
    SolType, UnionKind, declare_store::DeclareStore,
};
use ast_parser::fault::{AstErrorKind, EnumVariantArgumentTypeMismatch};
use sol_utils::{FunctionId, sol_names::PrimitiveTypes};
...
impl<'a> NameResolver<'a> {
    pub(crate) fn finish_call_resolution(
        ...
        if checkable {
            let mut generic_bindings: Vec<(&str, SolType)> = Vec::new();
            ...
            for (argument, parameter) in call.arguments.iter().zip(&parameters) {
                ...
                if matches!(parameter_ty, SolType::ImplTrait(_)) {
                    continue;
                }
    ...
    pub(crate) fn check_enum_variant_construction(
        ...
        let SolType::Stub(stub) = owner_type else {
            return;
        };
...
fn is_varargs_allowed_type(ty: &SolType) -> bool {
    use PrimitiveTypes::*;
    matches!(
        ty,
        SolType::RawPtr(_)
