use ast_model::{
    BlockId, ExpressionId, ExpressionKind, Generic, If, IfBranch, SolType, StatementKind,
};
...
impl<'a> NameResolver<'a> {
    ...
    fn check_return_value(
        ...
        match value {
            ...
            None => {
                if !matches!(return_type, SolType::None) {
                    self.log_error(
    ...
    fn check_tail_if(&mut self, if_: &If, return_type: &SolType, generics: &[Generic]) {
    ...
    pub(crate) fn first_lambda_return_type(&mut self, body: BlockId) -> Option<SolType> {
