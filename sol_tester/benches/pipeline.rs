...
// it's set to match `sol_tester::config::COMPILER_OPTIONS` anyway so this
...
fn fixture_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("sol")
}

/// Mirrors `sol_tester`'s own crate-store setup (see src/main.rs), pointed at the
/// `sol_tester/sol` fixture project so imports of `Std`/`Core` resolve.
fn build_crate_store(manifest_dir: &Path) -> CrateStore {
    ...
    for (name, spec) in deps {
        ...
        let source_root = sol_utils::collections::crate_store::resolve_source_root(&canonical);
        store.insert(
            name.clone(),
...
fn inputs() -> Vec<Input> {
    ...
    vec![
        Input {
            label: "tiny (main.sol, 5 lines)",
            source_folder: src.clone(),
            main_path: src.join("main.sol"),
            content: std::fs::read_to_string(src.join("main.sol")).unwrap(),
        },
        Input {
            label: "realistic (testCompiler.sol, 489 lines)",
            source_folder: src.clone(),
