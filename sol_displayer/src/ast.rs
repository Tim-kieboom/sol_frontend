...
use sol_utils::{
    ...
    ids::IdAlloc,
    linkage::Linkage,
    sol_names::{PrimitiveTypes, Symbol},
...
fn linkage_str(linkage: &sol_utils::linkage::Linkage) -> &'static str {
    match linkage {
...
impl<'a, W: Writer> Displayer<'a, W> {
    ...
    /// Resolves `id` through the interner and writes it — every `SolType`
    ...
    fn write_type_id(&mut self, id: TypeId) -> Result<()> {
        let ty = self
            ...
            .declares
            .get_type(id)
            .expect("SolType's internal TypeId fields are always interned");
        self.write_type(ty)
    }

    fn write_type(&mut self, ty: &SolType) -> Result<()> {
        match ty {
            SolType::TupleKind(kind) => {
                self.push_char('(')?;
            ...
            SolType::None => self.push_str(PrimitiveTypes::None.as_str()),
            SolType::Never => self.push_char('!'),
            SolType::Primitive(primitive_types) => self.push_str(primitive_types.as_str()),
            SolType::Array(array) => {
                match array.kind {
            ...
            SolType::Reference(reference) | SolType::Pointer(reference) => {
                if matches!(ty, SolType::Pointer(_)) {
                    self.push_char('*')?;
                } else {
                    self.push_char('&')?;
                }
            ...
            SolType::String => self.push_str(Types::String.as_str()),
            SolType::FormatString => self.push_str(Types::FormatString.as_str()),
            SolType::Any => self.push_str(Types::Any.as_str()),
            SolType::Error => self.push_str(Types::Error.as_str()),
            SolType::Function { arity, return_type } => {
                push_fmt!(self, "fn({arity} args) -> ")?;
                self.write_type_id(*return_type)
            }
            SolType::Type => self.push_str("type"),
        }
    }
