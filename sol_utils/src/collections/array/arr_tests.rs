use crate::collections::array::Arr;
...
#[test]
fn default_is_empty() {
    let arr: Arr<i32> = Arr::default();
    assert!(arr.is_empty());
}

#[test]
fn with_size_fills_every_slot_with_a_clone() {
    let arr = Arr::with_size(3, "x".to_string());
    assert_eq!(arr.len(), 3);
...
fn from_vec_preserves_order() {
    let arr = Arr::from_vec(vec![1, 2, 3]);
    assert_eq!(arr.len(), 3);
...
fn get_is_bounds_checked() {
    let arr = Arr::from_vec(vec![10, 20]);
...
fn from_vec_conversion_trait() {
    let arr: Arr<i32> = vec![1, 2, 3].into();
    assert_eq!(arr.as_slice(), [1, 2, 3]);
}

#[test]
fn from_iterator_collects_in_order() {
    let arr: Arr<i32> = (1..=3).collect();
    assert_eq!(arr.as_slice(), [1, 2, 3]);
}

#[test]
fn clone_is_a_deep_copy_independent_of_the_original() {
    let original = Arr::from_vec(vec![1, 2, 3]);
...
fn equality_compares_by_contents() {
    let a = Arr::from_vec(vec![1, 2, 3]);
    let b = Arr::from_vec(vec![1, 2, 3]);
    let c = Arr::from_vec(vec![1, 2, 4]);
    assert_eq!(a, b);
    assert_ne!(a, c);
}
...
fn serde_round_trips_through_json() {
    ...
    let round_tripped: Arr<i32> = serde_json::from_value(json).unwrap();
