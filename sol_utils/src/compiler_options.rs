...
/// Target-specific integer widths codegen needs to turn a Sol type like
...
pub struct PlatformInfo {
    /// Width of Sol's own platform-sized `int`/`uint` — pointer width.
...
impl PlatformInfo {
    ...
    pub const fn host() -> Self {
        if cfg!(all(target_os = "windows", target_arch = "x86_64")) {
            ...
            panic!(
                "Sol doesn't have a PlatformInfo for this (target_os, target_arch) combination yet — add one to PlatformInfo::host()"
            );
        }
    }
